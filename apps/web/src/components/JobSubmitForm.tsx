...
export function JobSubmitForm({ onSuccess }: JobSubmitFormProps) {
  ...
  return (
    <section
      className="card p-8 transition-all duration-500 ease-in-out"
      ...
      <div className="space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label
              htmlFor="image-url"
              className="block text-sm font-bold text-[hsl(var(--color-text))] mb-2"
            >
              Image URL *
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="input pl-12 pr-12 text-base font-medium"
                disabled={loading}
            ...
            {previewError && !error && (
              <div
                id="preview-error"
                className="flex items-start gap-2 p-3 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl"
                role="alert"
                aria-live="polite"
              >
                <svg
                  className="w-5 h-5 text-[hsl(var(--color-warning))] flex-shrink-0 mt-0.5"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  aria-hidden="true"
                >
          ...
          <div className="space-y-4 p-4 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl">
            ...
            <div>
              <label
                ...
                className="block text-xs font-semibold text-[hsl(var(--color-text))] mb-1.5"
              >
                Blur: {blur > 0 ? blur.toFixed(1) : "None"}
              </label>
              <input
                id="blur"
                type="range"
                ...
                {previewLoading && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="space-y-3 text-center">
                      <div
                        className="w-10 h-10 border-2 border-[hsl(var(--color-primary))] border-t-transparent rounded-full animate-spin mx-auto"
                        aria-hidden="true"
                      />
                ...
                {!previewLoading && !previewUrl && !previewError && (
                  <div className="absolute inset-0 flex items-center justify-center text-[hsl(var(--color-text-secondary))]">
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-sm">Enter a valid image URL</p>
                    </div>
                  </div>
                )}
          ...
          <button
            ...
            aria-label={loading ? "Submitting job..." : "Submit job"}
          >
            {loading ? (
              <>
                <div
        ...
        {showSuccess && (
          <div
            className="flex items-center gap-2 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl text-sm text-green-700 font-medium"
            role="status"
            aria-live="polite"
          >
            <svg
