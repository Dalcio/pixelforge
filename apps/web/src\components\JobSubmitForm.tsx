...
export function JobSubmitForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasTransformations) {
      setValidationError("Please select at least one transformation option to process the image.");
      return;
    }
  ...
  return <section className="card p-8 transition-all duration-500 ease-in-out" aria-labelledby="submit-form-title">
      <div className="space-y-6">
        <div className="text-center">
          ...
          <p className="text-[hsl(var(--color-text-secondary))]">{t("paste_your_image_url")}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative w-full aspect-video bg-gradient-to-br from-blue-100 to-cyan-100 rounded-2xl overflow-hidden">
                ...
                {!previewLoading && !previewUrl && !previewError && <div className="absolute inset-0 flex items-center justify-center text-[hsl(var(--color-text-secondary))]">
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-sm">{t("enter_a_valid_image")}</p>
                    </div>
                  </div>}
          ...
          {/* Submit Button */}
          <button type="submit" disabled={loading || !url.trim()} className="btn-primary w-full" aria-label={loading ? "Submitting job..." : "Submit job"}>
            {loading ? <>
